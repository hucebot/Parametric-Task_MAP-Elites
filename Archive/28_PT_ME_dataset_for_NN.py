...
if env == "talos":
    ...
    if method == "MT-ME":
        archives = [{"archive": Res[i]["archive"]} for i in names["MT-ME"]]
    else:
...
if method == "MT-ME":
    ...
    for i, archive in enumerate(archives):
        MR_Archives[(i,5000)] = {"elites": archive["archive"].elites}
    save_pickle(f"/home/pal/notebooks/data/tmp/MR_elites_{env}_{method}.pk", MR_Archives)
else:
    Args = {}
...
MR_Archives = open_pickle(f"/home/pal/notebooks/data/tmp/MR_elites_{env}_{method}.pk")

for (i, res), archive in tqdm(MR_Archives.items()):
    samples = []
    for elite in archive["elites"]:
        samples.append({"situation": elite["situation"], "command": elite["command"], "it": elite["id"]})
    save_pickle(f"/home/pal/notebooks/data/PT-ME/training_sets/{env}/{method}_{i}_{res}.pk", samples)
